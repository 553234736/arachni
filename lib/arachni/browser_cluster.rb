...
module Arachni
  ...
  class BrowserCluster
    ...
    #   将每个浏览器的生命周期限制为给定的页面数量。
    #   超过该数量时，将终止当前进程并将新进程推送到池中，有助于防止内存泄漏问题。
    ...
    def with_browser(*args, &block)
      method_handler = nil
      method_handler = args.pop if args.last.is_a? Method
    ...
    def queue(job, cb = nil, &block)
      ...
      synchronize do
        ...
        unless @job_callbacks[job.id]
          raise ArgumentError, "No callback set for job ID #{job.id}."
        end
    ...
    def job_done?(job, fail_if_not_found = true)
      ...
      synchronize do
        ...
        return false unless @pending_jobs.include?(job.id)
    ...
    def notify_on_job_done(job)
      return unless @on_job_done

      @on_job_done.call job
    end

    def notify_on_pop(job)
      return unless @on_pop

      @on_pop.call job
    end

    def fail_if_shutdown
      raise Error::AlreadyShutdown, "Cluster has been shut down." if @shutdown
    end

    def fail_if_job_done(job)
      return unless job_done?(job, false)

      raise Job::Error::AlreadyDone, "Job has been marked as done."
    end

    def fail_if_job_not_found(job)
      return if @pending_jobs.include?(job.id)

      raise Error::JobNotFound, "Job could not be found."
    end
