  ...
  module Check
    ...
    class Manager < Arachni::Component::Manager
      ...
      def [](name)
        ...
        unless Platform::Manager.valid?(check.platforms)
          unload name
          raise Error::InvalidPlatforms,
      ...
      def schedule
        ...
        preferred_over.size.times do
          ...
          preferred.each do |name, klass|
            schedule << klass
            preferred_over.select { |_, v| v.include?(name.to_sym) }.each do |k, _v|
      ...
      def run_one(check, page)
        return false unless check.check?(page)
